...
function renderSupplementaryResearch(container) {
  const section = el(
    '<details class="supplementary-block">' +
      '<summary>Supplementary research <span>Optional</span></summary>' +
      '<div class="supplementary-content">' +
        '<p class="hint">Open only when the conversation moves into event value, measurement or concept exploration. Leave anything not discussed blank.</p>' +
        '<h3>Decision context</h3>' +
        '<div class="field"><label>Responsible for event reporting</label><div id="f-eventReporting" class="chip-row"></div></div>' +
        '<div class="field"><label>Involved in sponsorship purchasing or renewal</label><div id="f-sponsorshipPurchasing" class="chip-row"></div></div>' +
        '<h3>Value and measurement</h3>' +
        '<div class="field"><label>Success outcomes mentioned</label><div id="f-successOutcomes" class="chip-row"></div></div>' +
        '<div class="field"><label>When success becomes visible</label><div id="f-successVisibility" class="chip-row"></div></div>' +
        '<div class="field"><label>Evidence currently used</label><div id="f-currentEvidence" class="chip-row"></div></div>' +
        '<div class="field"><label for="f-valueMeasurementNote">Clarification note</label><input id="f-valueMeasurementNote" type="text" placeholder="Optional, particularly for Other" /></div>' +
        '<h3>Concept exploration</h3>' +
        '<div class="field"><label>Concepts discussed</label><div id="f-conceptsDiscussed" class="chip-row"></div></div>' +
        '<div id="f-conceptEvidence"></div>' +
      '</div>' +
    '</details>'
  );
  container.appendChild(section);

  makeChipField(document.getElementById('f-eventReporting'), EVENT_REPORTING_RESPONSIBILITY, false,
    () => currentEntry.responsibleForEventReporting,
    (value) => currentEntry.responsibleForEventReporting = value);
  makeChipField(document.getElementById('f-sponsorshipPurchasing'), SPONSORSHIP_PURCHASING_INVOLVEMENT, false,
    () => currentEntry.involvedInSponsorshipPurchasing,
    (value) => currentEntry.involvedInSponsorshipPurchasing = value);
  makeChipField(document.getElementById('f-successOutcomes'), SUCCESS_OUTCOMES, true,
    () => currentEntry.successOutcomes,
    (value) => currentEntry.successOutcomes = value);
  makeChipField(document.getElementById('f-successVisibility'), SUCCESS_VISIBILITY_TIMINGS, false,
    () => currentEntry.successVisibility,
    (value) => currentEntry.successVisibility = value);
  makeChipField(document.getElementById('f-currentEvidence'), CURRENT_EVIDENCE_TYPES, true,
    () => currentEntry.evidenceCurrentlyUsed,
    (value) => currentEntry.evidenceCurrentlyUsed = value);

  const note = document.getElementById('f-valueMeasurementNote');
  note.value = currentEntry.valueMeasurementNote || '';
  note.addEventListener('input', () => {
    currentEntry.valueMeasurementNote = note.value;
    scheduleSave();
  });

  const conceptsContainer = document.getElementById('f-conceptsDiscussed');
  const evidenceContainer = document.getElementById('f-conceptEvidence');

  function paintConceptEvidence() {
    evidenceContainer.innerHTML = '';
    (currentEntry.conceptsDiscussed || [])
      .filter((concept) => concept !== 'none')
      .forEach((concept) => {
        const definition = CONCEPTS.find((item) => item.value === concept);
        if (!definition) return;
        if (!currentEntry.conceptEvidence[concept]) currentEntry.conceptEvidence[concept] = {};
        const conceptData = currentEntry.conceptEvidence[concept];
        const card = el(
          '<section class="concept-card">' +
            '<h4></h4>' +
            '<div class="field"><label>Existing problem evidenced</label><div class="chip-row concept-problem"></div></div>' +
            '<div class="field"><label>Current workaround<input class="concept-workaround" type="text" placeholder="Optional" /></label></div>' +
            '<div class="field"><label>Important condition, risk or objection<input class="concept-condition" type="text" placeholder="Optional" /></label></div>' +
            '<div class="field"><label>Evidence of value the participant would expect<input class="concept-value" type="text" placeholder="Optional" /></label></div>' +
          '</section>'
        );
        card.querySelector('h4').textContent = definition.label;
        evidenceContainer.appendChild(card);

        makeChipField(card.querySelector('.concept-problem'), EXISTING_PROBLEM_EVIDENCE, false,
          () => conceptData.existingProblemEvidenced || '',
          (value) => conceptData.existingProblemEvidenced = value);

        [
          ['.concept-workaround', 'currentWorkaround'],
          ['.concept-condition', 'conditionRiskObjection'],
          ['.concept-value', 'expectedValueEvidence']
        ].forEach((binding) => {
          const input = card.querySelector(binding[0]);
          input.value = conceptData[binding[1]] || '';
          input.addEventListener('input', () => {
            conceptData[binding[1]] = input.value;
            scheduleSave();
          });
        });
      });
  ...
  function paintConceptSelector() {
    renderChipGroup(conceptsContainer, CONCEPTS, currentEntry.conceptsDiscussed, true, (nextValues) => {
      const previousValues = currentEntry.conceptsDiscussed || [];
      let normalised = nextValues;
      if (nextValues.includes('none') && !previousValues.includes('none')) {
        normalised = ['none'];
      } else if (previousValues.includes('none') && nextValues.length > 1) {
        normalised = nextValues.filter((value) => value !== 'none');
      }
      previousValues
        .filter((value) => value !== 'none' && !normalised.includes(value))
        .forEach((value) => { delete currentEntry.conceptEvidence[value]; });
      if (normalised.includes('none')) currentEntry.conceptEvidence = {};
      currentEntry.conceptsDiscussed = normalised;
      scheduleSave();
      paintConceptSelector();
      paintConceptEvidence();
    });
  }
...
function renderEntryForm(entry, mode) {
  ...
  container.appendChild(el(
    '<div class="field">' +
      '<label>Evidence classification <span class="optional-label">Optional</span></label>' +
      '<p class="hint">Classify what this entry represents. Proposed concepts are not current behaviour.</p>' +
      '<div id="f-evidenceClassification" class="chip-row"></div>' +
    '</div>'
  ));
  makeChipField(document.getElementById('f-evidenceClassification'), EVIDENCE_CLASSIFICATIONS, false,
    () => currentEntry.evidenceClassification,
    (value) => currentEntry.evidenceClassification = value);
  ...
  renderSupplementaryResearch(container);
...
async function renderSavedList() {
  ...
  async function paintList() {
    ...
    const rows = all
      ...
      .filter((e) => {
        if (!q) return true;
        const conceptText = Object.values(e.conceptEvidence || {}).map((concept) => [
          concept.currentWorkaround,
          concept.conditionRiskObjection,
          concept.expectedValueEvidence
        ].join(' ')).join(' ');
        const haystack = [
          e.directEvidence, e.recentExample, e.confirmedExplanation,
          e.interpretation, e.contextLimitation, e.legacyNotes,
          e.valueMeasurementNote, conceptText
        ].join(' ').toLowerCase();
...
async function boot() {
  try {
    ...
    if ('caches' in window) {
      ...
      await Promise.all(
        cacheNames
          .filter((name) => name.startsWith('curfel-') && name !== 'curfel-v7')
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('sw.js?v=7').catch(() => { /* still usable online-only this run */ });
